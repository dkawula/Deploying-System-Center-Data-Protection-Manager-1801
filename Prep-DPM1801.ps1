 ...
 param
  (
   ...
   # Add-LocalGroupMember -Group Administrators -Member $DPMServiceAcct
    ...
    Write-Output -InputObject "[$($VMName)]:: Adding Drives for Storage Spaces / Backup Target"
    New-VHD -Path "$($VMPath)\$($GuestOSName) - DPM SS 1.vhdx" -Dynamic -SizeBytes 200GB 
    New-VHD -Path "$($VMPath)\$($GuestOSName) - DPM SS 2.vhdx" -Dynamic -SizeBytes 200GB 
    New-VHD -Path "$($VMPath)\$($GuestOSName) - DPM SS 3.vhdx" -Dynamic -SizeBytes 200GB 
    New-VHD -Path "$($VMPath)\$($GuestOSName) - DPM SS 4.vhdx" -Dynamic -SizeBytes 200GB 
    New-VHD -Path "$($VMPath)\$($GuestOSName) - DPM SS 5.vhdx" -Dynamic -SizeBytes 200GB 
    New-VHD -Path "$($VMPath)\$($GuestOSName) - DPM SS 6.vhdx" -Dynamic -SizeBytes 200GB 
    Add-VMHardDiskDrive -VMName $VMName -Path "$($VMPath)\$($GuestOSName) - DPM SS 1.vhdx" -ControllerType SCSI
    Add-VMHardDiskDrive -VMName $VMName -Path "$($VMPath)\$($GuestOSName) - DPM SS 2.vhdx" -ControllerType SCSI
    Add-VMHardDiskDrive -VMName $VMName -Path "$($VMPath)\$($GuestOSName) - DPM SS 3.vhdx" -ControllerType SCSI
    Add-VMHardDiskDrive -VMName $VMName -Path "$($VMPath)\$($GuestOSName) - DPM SS 4.vhdx" -ControllerType SCSI
    Add-VMHardDiskDrive -VMName $VMName -Path "$($VMPath)\$($GuestOSName) - DPM SS 5.vhdx" -ControllerType SCSI
    Add-VMHardDiskDrive -VMName $VMName -Path "$($VMPath)\$($GuestOSName) - DPM SS 6.vhdx" -ControllerType SCSI  
